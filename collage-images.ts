...
export async function loadRandomImages() {
  ...
  for (const [index, imageCollage] of Array.from(
    document.querySelectorAll<HTMLDivElement>(".image-collage"),
  ).entries()) {
    ...
    const totalNormalizedWidth = images.reduce(
      (sum, img) => sum + img.normalizedWidth,
      0,
    );
...
document.addEventListener("DOMContentLoaded", () =>
  loadRandomImages().catch(console.error),
);
